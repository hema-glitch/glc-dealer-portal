/**
 * zohoBooks.js — Zoho Books API wrapper with cache.
 * Uses /tmp for cache on Vercel (only writable directory in serverless).
 * Falls back to in-memory cache if file write fails.
 */
...
// Use /tmp on Vercel, or local .cache.json in development
...
async function getInvoicesForDealer(contactId) {
  ...
  const results  = await Promise.all(
    statuses.map(status =>
      zbGet('/invoices', { customer_id: contactId, status, sort_column: 'date', sort_order: 'D', per_page: 200 })
  ...
  console.log('[ZohoBooks] Fetched', unique.length, 'invoices');
...
async function getPaymentsForDealer(contactId) {
  ...
  try {
    const data     = await zbGet('/customerpayments', { customer_id: contactId, sort_column: 'date', sort_order: 'D' });
...
async function getSalesOrdersForDealer(contactId) {
  try {
    const data = await zbGet('/salesorders', { customer_id: contactId, sort_column: 'date', sort_order: 'D' });
